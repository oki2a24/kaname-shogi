...
最終整理：2026-09-18。恒常的な運用方針はルートのAGENTS.mdを参照する。

## 現在の到達点

初期配置CLIと歩・金・銀・香・飛車の移動先候補を実装。飛車は第14回で学習・設計合意を行い、元の `/Users/oki2a24/kaname-shogi` のmainで実装した。別作業ツリーからの取り込みは不要。飛車の実装とレビュー対応後、全64テスト、CLI、`git diff --check`、文書参照の確認が成功し、実装コミットは `976e0c1`、理解確認と再開案内の更新は `8f617f6`。現在のGit状態はクリーンである。
...
飛車は学習・設計合意後、元のmainへ実装した。`rook_move_candidates(board, source)`は右・左・前・後ろの順に候補を返す。Redでは未定義によるImportErrorと、ValueError契約の失敗を区別した。Greenでは最小実装で4方向を走査し、追加テストを含む全64テストが成功した。Refactorでは方向データと走査処理、変数名、公開docstringを点検し、香との共通化は先取りしなかった。レビュー指摘により盤端の四隅の正確な候補座標・順序を検証するテストと、モジュールdocstring・設計文書参照を追加した。実装後はコードを説明し、4方向の外側`for`と各方向の内側`while`、相手駒のマスを追加する`candidates.append(target)`を確認した。本人は「ほかに確認したい点はありません」と振り返った。実装・検証記録は[第15回](learning/15-rook-candidates-implementation.md)を参照する。次のテーマは角の移動先候補に決定したが、学習・設計・実装はまだ開始していない。
...
1. 現在のGit状態とREADME、角テーマに関係する最新記録を確認する。
2. 角の動きを日本将棋連盟の一次情報で確認し、学習記録を作成する。
3. 確認問題を一問ずつ出し、本人の回答と補足を分けて記録する。
4. 学習後に`superpowerssuperpowers:brainstorming`で角の対象範囲・候補順・方向表現・処理共有・確認方法を相談する。
5. 設計承認後、`test-driven-development`を読み、Red → Green → Refactorで角の候補生成を実装する。成り、実際の移動、駒取り、持ち駒、王手、合法手判定は先取りしない。
...
飛車の移動先候補は学習・設計・実装・検証・レビュー・理解確認まで完了しています。次のテーマは角の移動先候補です。角の動きを日本将棋連盟の一次情報で確認し、学習から始めてください。確認問題は一問ずつ出し、回答を待ってください。まだ設計・実装には進まないでください。
