...
第35回「終局理由の拡張」は完了し、mainへの取り込みと取り込み先での検証、最後の理解確認まで終わった。次テーマは未選定で、候補と推薦理由は[次のテーマ候補](next-topics.md)を参照する。
...
最終整理：2026-09-23。恒常的な運用方針はルートのAGENTS.mdを参照する。現在はmainで第35回まで完了している。次テーマが選ばれるまで、新しい学習・実装には進まない。
...
1. [次のテーマ候補](next-topics.md)を確認し、本人が次テーマを選ぶまで待つ。
2. 次テーマが選ばれ、新しいセッションで進める場合は、方針に従って引き継ぎ文書を作成する。
