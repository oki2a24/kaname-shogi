...
最終整理：2026-09-21。恒常的な運用方針はルートのAGENTS.mdを参照する。
...
次のテーマは「手番更新」。今回の `move_piece(board, source, destination)` は盤面だけを変更し、`Position.side_to_move` は変更しない。次回は、`Board` が盤面データ、`Position` が盤面と手番を組み合わせた局面であることを再確認し、移動成功時だけ手番を先手・後手で交代する最小範囲を学ぶ。不正な移動時には盤面と手番を変更しない。駒取り、持ち駒、成り、王手、合法手判定、CLI入力、評価、探索は扱わない。
...
次のテーマは「手番更新」です。前回の空マスへの移動適用は `06292b7` で完了しており、次回はBoardとPositionの責務、成功時だけ手番を更新する設計を学びます。
