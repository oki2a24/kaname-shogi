...
2. 第22回の最終検証、レビュー、文書の整合確認、コミット状態を確認する。
3. 第22回の実装後理解確認と振り返りを行う。
4. 理解確認と振り返りが完了するまで、駒取り・持ち駒・成りの次テーマへ進まない。

## 次回セッションの開始点

手番更新は実装した。次回は、`move_piece` が盤面専用、`apply_move` が成功した移動と手番交代を局面へ適用する操作であることを理解確認する。不正な移動では盤面と手番がともに変わらず、今回は手番と出発駒の所有者の一致を検証しない。駒取り、持ち駒、成り、王手、合法手判定、CLI入力、評価、探索は扱わない。
