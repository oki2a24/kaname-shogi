...
- `Board` は可変の81要素の一次元リスト、空マスは `None`。`Position` は可変で盤面と手番を保持する。
- 成り状態・持ち駒などは必要な回に追加する。
- 筋・段を基本とし、内部だけで `index = (file - 1) * 9 + (rank - 1)` に変換する。変換は `Square.to_index()` に集約する。１一は0、９九は80。
- 保存順は１筋の一〜九段から順に並べる。表示順は段が一→九、筋が9→1。初期配置やテストは筋・段で記述する。
